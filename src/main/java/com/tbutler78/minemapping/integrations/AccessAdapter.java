...
public class AccessAdapter {
  ...
  public AccessTable getResultSet(String query, int maxRows) {
    ...
    try {
      ...
      log.error("Problem in loading or " + "registering MS Access JDBC driver");
    ...
    try {
      ...
      connection =
          DriverManager.getConnection(
              DB_URL); // Create and get connection using DriverManager class
      ...
      while (resultSet.next() && counter < maxRows) {
        ...
        for (int i = 1; i <= columns; i++) {
          result.put(
              rsmd.getColumnName(i), resultSet.getString(i) != null ? resultSet.getString(i) : "");
        }
